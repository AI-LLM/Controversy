# 2026-05-31：AI 的「Dark Output」——为什么 AI 创造的价值可能在GDP中不可见
...
⚠ 本文的核心评判（见文末「综合判断」）：**原报告这条主论点（PO1）至今没有任何可验证的事实支撑，全部建立在类比与结构性推论之上**——它赖以立论的论据（Solow 悖论、2013 年 GDP 修订、服务业统计口径、马力类比、消费者剩余理论）无一例外是历史先例与经济学理论，没有一条能落在原文自创的验证阶梯（T4+）上。而把天平推向「相当部分支出其实是浪费」的，反倒是一批可验证的产业实测（Waydev、GitClear、Uber、Meta／Amazon）。**于是原报告恰好输在了它自己提出的证据标准上。**
...
- **边的颜色**：只有**落在验证阶梯上（T4+）的论据**，其极性边才着色——**绿色＝支持、红色＝反对**；离阶梯（N/A）论据的边保持默认灰色。这样一眼可见「有部署强度／实测背书的证据」各自把哪个立场往哪个方向推（评级见文末「用验证阶梯给论据评级」一节）。
...
graph TD
    ...
    %% 阶梯评级（T4+）论据的极性边着色：支持=绿、反对=红（边按出现顺序 0 起索引）
    %% 绿（支持）：AR3→PO7(20)、AR4→PO15(39)、AR17(45)、AR18(46)、AR21(49)、AR23(51)、AR24(52)、AR25(53)、AR26→PO15(54)、AR27→PO15(55)、AR28(56)、AR26→PO17(57)、AR27→PO17(58)
    linkStyle 20,39,45,46,49,51,52,53,54,55,56,57,58 stroke:#2e7d32,stroke-width:2.5px;
    %% 红（反对）：AR3→PO1(21)、AR4→PO7(22)、AR13→PO15(42)
    linkStyle 21,22,42 stroke:#c62828,stroke-width:2.5px;
```
...
- **支持 PO1 的论据**：
  ...
  - （注：AR13「持续付费／revealed preference」**不支持 PO1**——它只对 PO15 起反驳作用，理由见 PO15 一节。关键区别：AR12 指向**价格之外的消费者剩余**（不入账，故确为 Dark Output）；AR13 指向**付费动作本身**，而企业向 Anthropic 的采购恰恰已计入 GDP、是可见的，无法佐证「价值不可见」。）
- **反对 PO1 的论据**：
...
- **【PO7】立场**：头条数字 **1.53 万亿美元**是该阶梯 **T4 及以上**的「已验证边界」劳动暴露（verified-boundary labor exposure，按锚定每个 DWA 的证据类型划分，共 192 个 DWA）[2]。它**不是**说 1.53 万亿美元的劳动已经消失，而是说与该规模劳动成本挂钩的任务落在「当代 AI 具可信替代潜力」的类别里——应读作**暴露劳动（exposed labor）**，而非缺失产出。
  - **【AR3｜支持 PO7·反对 PO1｜证据 T4+】** 这 1.53 万亿美元的构成恰恰指向 AI 增强而非替代：**增强 Augmentation 9520 亿美元（62.3%，133 个 DWA）**、**混合 Mixed 5620 亿美元（36.8%，56 个 DWA）**、**纯替代 Replacement 仅 140 亿美元（0.9%，3 个 DWA）**[2]。新闻标题给人「AI 取代人类（Human → AI）」的印象，数据却是「让员工更强（Human + AI）」占绝对主导。⚠ **此条同时反驳 PO1**：这 1.53 万亿是用 **BLS 工资**度量、且 62.3% 是增强型（工人留在岗位、产出照旧开账单），因此它**整块都落在已计入 GDP 的可见经济里**，不是「不可见的暗产出」。换言之，作者据以论证 Dark Output 规模的头条数字，其主体恰恰是**被账户看得见的**那部分——它能支持「这是暴露劳动」（PO7），却反过来削弱「价值不可见」（PO1）。只有当增强**日后转为替代**、收据真的消失时，它才会滑入暗产出；就当下的 T4+ 读数而言，它是可见的。
...
这正体现了本图的建模原则：**论据是中立节点，支持/反对在边上**。同一条论据常常一身二任——AR9、AR12 支持 PO1 的同时反对 PO15；AR14、AR4 则反过来。（反例：AR13 虽也反对 PO15，却**不**支持 PO1——付费本身计入 GDP、属可见，无法佐证「不可见」。）
...
2. **【AR13｜反对 PO15｜证据 T4】持续付费是 revealed preference**。用户续订、扩大用量、企业重复采购，若纯属浪费理应快速流失；Gary Marcus 自己也承认 Anthropic「本季度仍在盈利」[[5]](https://garymarcus.substack.com/p/what-happens-next-after-the-decline)。⚠ **此条只反驳 PO15，不支持 PO1**：企业向 Anthropic 的付费属 B2B 采购、本就计入 GDP（可见），因此「有人持续付费」证明的是「价值存在」、而非「价值不可见」——它落在阶梯 T4（真实部署、有钱在流动）这一**可见**层级，恰恰不是 PO1 主张的暗产出。⚠ 另一限定：它对「被捕获型／订阅制」强，对「内部 token／新增型／补贴驱动的刷量」弱——后者恰恰是浪费假说的主场。
...
**先说最硬的一条结论：原报告主论点 PO1 没有可验证的事实支撑，只有类比推论。** 把全部论据按原文自创的验证阶梯过一遍（见上节、并见图中边的着色），结果是结构性的、而非偶然的：

- **支持 PO1 的论据共 4 条——AR9（Solow 悖论）、AR10（服务业统计口径）、AR11（马力类比）、AR12（消费者剩余理论），无一落在阶梯上（全部 N/A）。** 它们要么是历史先例、要么是统计原理、要么是经济学理论与类比，没有一条是「AI 在真实部署中确实产生了看不见的价值」的直接证据。图上 PO1 没有一条带色（T4+）的支持边。
- **反过来，唯一与 PO1 相关的带色边是红的**：AR3（1.53 万亿、用 BLS 工资度量、主体为留岗增强）反驳 PO1——因为这块产出整体计入 GDP、是可见的。
- 这不是取证不力，而是**结构必然**：暗产出之所以"暗"，就是它不留下任何可被阶梯捕捉的信号。所以 PO1 在设计上就**不可证伪**——它永远可以把任何"看不见"解释为"隐形价值"。

与之对照，**指向「相当部分是浪费」（PO15）的一侧握有可验证的实测**：AR26（Waydev：表面接受 80–90% → 真实留存 10–30%）、AR27（GitClear：churn 9.4 倍）是对价值兑现的**直接测量**，AR23（Uber 烧光预算、COO 称画不出价值线）、AR24（Meta／Amazon 刷量）、AR17／AR18／AR21 是 T4 级部署的真实读数。它们在编程这一最大 token 类别上，把天平明确推向「浪费」。

**所以最站得住的结论不是把 PO1 和 PO15 等量齐观的折中。** 在原报告自己设定的证据标准（验证阶梯）下：PO1 拿不出任何阶梯级证据、只有类比；PO15 拿得出测量级证据。因此——

- 整体仍是 PO16 式的混合（**缺口 = 隐形真实价值 + 浪费支出 + 资本错配**，比例未知、随补贴退坡与技术成熟而变），但**混合的举证天平是倾斜的**：在已能测量的编程域，证据偏向浪费；在尚不能测量的领域（被捕获型、消费者剩余），PO1 也只是"未被证伪"、而非"已被证实"。
- **举证责任落在「价值已实现」一方**。原报告若要成立，需要的恰是它自己承认尚不充分、且至今全空的 T5–T6 证据（AR4），而不是「统计看不见」这一不可证伪的结构性论证（AR14）。在这条标准达成之前，应把 Dark Output 视为**一个尚待检验的假说**，而非已确立的事实——而当下能拿到的可验证事实，反倒更多地站在它的对立面。

## 用验证阶梯（T0–T6）给论据评级

⚠ 适用范围：验证阶梯（AR2）原是评 **AI 输出／部署被验证到何种强度** 的**正向**尺子，因此它只能评「关于 AI 在真实世界部署」这一类论据。本图多数论据是逻辑、历史、经济理论或价格证据，**不在阶梯上**（N/A）。

⚠ 关键限度（本人判断）：**验证阶梯量的是「验证强度」这一根轴，它与另外两根轴正交、并不能替代它们**——

1. **验证强度**（T0–T6）：这个部署被核验到多硬？（阶梯量的就是它）
2. **可见性**（PO1／PO15 之争的轴）：价值是否计入 GDP？还是不可见（暗产出）？
3. **价值兑现**（PO15 的轴）：净价值真的产生了，还是被浪费？

阶梯只量第 1 根。更要命的是第 1 根与第 2 根之间有一条**结构性锁死**：**T4 的定义是「真实部署 + 有钱在流动」，而「有钱在流动」＝一笔被记录的交易＝计入 GDP（至少是 AI 厂商的收入）。所以任何 T4 及以上的证据，按定义都属于「可见经济」**。由此得到一条判据：

> **T4+ 证据在结构上不可能支持 PO1（价值不可见）**——它要么反驳 PO1（证明价值是被看得见、被记账的），要么只对 PO15（可见经济内部「是真价值还是浪费」）说话。能支持 PO1 的，只可能是离阶梯的结构性论据。

**落在阶梯上的论据**

| 论据 | 极性边 | 层级 | 依据 |
| --- | --- | --- | --- |
| AR3 | 支持 PO7·反对 PO1 | **T4+** | 1.53 万亿按 T4+ 口径汇总，且 62.3% 为增强（工人留岗、产出照旧开账单）；⚠ 用 BLS 工资度量、整块计入 GDP（可见），故支持「暴露劳动」（PO7）却反驳「价值不可见」（PO1） |
| AR13 | 反对 PO15 | **T4** | 持续付费、续订、复购 = 真实业务流程中创收的部署（revealed preference）；⚠ 付费本身计入 GDP（可见），故只反驳「浪费」、不支持「不可见」（PO1） |
...
**评级浮现的判断** ⚠（本人解读）：先看一个被阶梯结构锁死的事实——**落在阶梯上的论据没有一条支持 PO1**。这不是偶然：T4+ 按定义就是「有钱在流动＝计入 GDP＝可见」，可见的东西不可能佐证「不可见」。AR3（1.53 万亿、用 BLS 工资度量、主体是留岗增强）、AR13（持续付费）都因此**反驳**而非支持 PO1。支持 PO1 的四条（AR9 历史、AR10 统计口径、AR11 类比、AR12 消费者剩余理论）**全部离阶梯**——这恰恰印证：暗产出之所以叫暗，就是它不会留下任何可被阶梯捕捉的信号。

再看 PO1 与 PO15 在阶梯内部的对决。**T4 只意味着「真实部署 + 有钱在流动」，不等于价值已兑现**——所以可见经济内部仍是混战：AR3／AR13 证明 AI 被广泛采用并付费（＝不是纯浪费），AR17／AR18／AR21／AR23／AR24／AR25 证明「用了、付了 ≠ 值了」。能真正裁定价值是否兑现的 **T5（经裁决）、T6（承保）两级至今全空**（AR4）。
