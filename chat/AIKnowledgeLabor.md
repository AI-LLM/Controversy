...
§3.2.1 回答了"需求在哪里"；接下来的问题是——找到需求之后，怎样运用 AI 劳动力满足需求。§3.2.1.6 末尾已经给出总纲：一旦把 Agent 建模为"AI 劳动力"而非"自主软件系统"，组织它的就不再是某种尚未发明的"自动编程理论"，而是早已成熟的管理学与工程学方法论——派活、定规范、设质检、做监督。本节以最典型、训练数据最丰富、当前 AI 能力最强的知识工作——**软件开发**——为标本，对比传统流程，提炼这套运用方法。
