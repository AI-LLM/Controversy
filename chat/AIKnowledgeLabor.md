...
- [二、LLM Agent 的能力轮廓：为什么它是"劳动力"，而不是工具](#二llm-agent-的能力轮廓为什么它是劳动力而不是工具)
  ...
  - [2.7 "非工具"本质的数学论证](#27-非工具本质的数学论证)
    - [2.7.1 纯 LLM 不可能达到比特级确定性](#271-纯-llm-不可能达到比特级确定性)
    - [2.7.2 错误率可以被数学控制吗](#272-错误率可以被数学控制吗)
      - [2.7.2.1 PAC 学习理论：概率近似正确](#2721-pac-学习理论概率近似正确)
      - [2.7.2.2 标度律：错误率与算力/参数量的幂律](#2722-标度律错误率与算力参数量的幂律)
      - [2.7.2.3 置信度校准：让"模型说自己 90% 有把握"真的意味着 90%](#2723-置信度校准让模型说自己-90-有把握真的意味着-90)
      - [2.7.2.4 多次采样 + 验证器：错误率指数级衰减](#2724-多次采样-验证器错误率指数级衰减)
    - [2.7.3 纠错的计算量非对称性：必须靠"传统骨骼"重建](#273-纠错的计算量非对称性必须靠传统骨骼重建)
  ...
  - [2.8 IT 产业的反向印证：传统计算机正在成为 AI 的"骨骼"](#28-it-产业的反向印证传统计算机正在成为-ai-的骨骼)
    ...
    - [2.8.3 边缘 AI 与端侧大模型：传统 PC 组件被迫"内卷升级"](#283-边缘-ai-与端侧大模型传统-pc-组件被迫内卷升级)
    - [2.8.4 存储芯片短缺：AI 算力胃口挤压消费电子供应](#284-存储芯片短缺ai-算力胃口挤压消费电子供应)
  - [2.9 成本结构：教育投入前置，单次使用相对廉价](#29-成本结构教育投入前置单次使用相对廉价)
  - [2.10 AI 能力的"锯齿状边界" (Jagged Frontier)](#210-ai-能力的锯齿状边界-jagged-frontier)
...
##### 2.7.2.3 置信度校准：让"模型说自己 90% 有把握"真的意味着 90%
...
#### 2.7.3 纠错的计算量非对称性：必须靠"传统骨骼"重建

##### 2.7.3.1 传统计算机的天然优势：纠错远比求解便宜
...
##### 2.7.3.2 LLM 原生世界的窘境：纠错 ≈ 重新求解

LLM 每次前向传播都是一次密集矩阵乘法。要求 LLM "检查并纠正你的错误"，它必须把先前的上下文（含错误答案）作为输入再跑一遍完整的前向传播——计算量没有减少，反而**翻倍**。结合 §2.7.1 提到的"模型无法跳出自己的概率分布做裁判"，自我纠错经常变成"越改越错"，白白消耗双倍算力[[24]](https://arxiv.org/abs/2310.01798)。

##### 2.7.3.3 当前工业界如何"恢复"非对称性
...
"LLM 像人"很容易被误读为"AI 会让传统计算机退场"。**事实正相反**：这股新劳动力越像人、越无处不在，对传统计算机基础设施的拉动越疯狂。这是 §2.5–§2.7.3 的逻辑（会犯错、不能自我纠错、纠错计算量同等昂贵、必须靠传统骨骼）在 2025–2026 年 IT 产业资本流向上的物质投影。

#### 2.8.1 服务器 CPU：作为"宿主控制核心"的需求暴增
...
#### 2.8.2 Hyperscaler 资本开支：四家 2025 年合计 3,000–3,800 亿美元
...
#### 2.8.3 边缘 AI 与端侧大模型：传统 PC 组件被迫"内卷升级"
...
| | 生产成本（一次性） | 复制成本（扩产能） | 运行成本（每任务） |
|---|---|---|---|
| 传统软件 | 软件开发：高，但相对AI训练较低 | 免费（拷贝程序） | ≈ 免费（执行） |
| 人类劳动力 | 人类学习：多年教育，高 | 昂贵（无法拷贝，每人从头学一遍） | 相对昂贵（工资） |
| AI 劳动力 | 模型训练：数千万–数亿美元，高 + 应用适配：Harnessing，≈软件开发 | 免费（拷贝权重） | 相对廉价（推理计件） |
...
2026年开始流行的Harness engineering概念也可以被建模为一个综合了人力、AI劳动力和传统计算工具的系统工程，根据AI劳动力在具体任务上的差异化能力来调整工作流程，适配计算工具。基于以上论证的不确定性、成本结构和能力模型，可以得到新的视角和方法来分析和设计AI系统，§3.2.1.6给出了一个示例。
...
**C 类的护城河有七种，耐久性各不相同。** 把前面说的"残余护城河"拆开，大致是七类——共同点是归集方掌握了一样**个体各自复制不来、AI 也不自带**的东西：

- **C1 担责兜底**：有主体为错误或结果负责（诊断、税务稽查、合同 E&O）——AI 不能在责任书上签字。
- **C2 持牌·授权签字**：输出必须由有法定资格者盖章才生效——既包括个人执业牌照（律师、CPA、建筑师、医生），也包括组织层面的制度授权（谁有资格颁布被政府或行业认可的标准、出具有法定效力的认证）。牌照与授权本身就是壁垒。
- **C3 数据网络·独占输入**：护城河是别人拿不到的数据——可以是汇集多方才存在的网络数据（跨机构反诈联防、细分行业收费基准、交易对手赖账评分，越多买家越准），也可以是对单一稀缺源的独占访问（独家数字化的古籍语料、退休老师傅未公开的维修日志）。前者靠归集、后者靠独占，个体都拿不到全局。
- **C4 双边归集**：同时把分散的需求与供给聚到一处运营，靠跨边网络效应运转（单干家政 / 诊所的 AI 后台与前台、中小企业集采议价）。
- **C5 归集即立案**：单个诉求太小、不值得单独处理，聚合本身才创造价值（群体索赔、大规模仲裁、集体谈判）。
- **C6 中立鉴真**：交易双方都信任的第三方做鉴定或核查（二手奢侈品与车况鉴真、租户与雇员背景核查）。
- **C7 纵向数据锁定**：护城河是对同一用户**跨时间**积累的行为 / 认知档案——学习轨迹、用药依从史、口味与生物反应——越用越准、切换成本越高（个人技能教练、慢病管理、精准营养）。与 C3 的分野：C3 横向跨买家、数据离开你仍对所有人有用；C7 纵向深耕单人、数据主要对本人有价值。

判断一个 C 是"真 C"还是"迟早塌回 B"，有一个干脆的判据：**假设人人手里都有一个免费的天才模型，这门生意还有没有存在的理由**？经得起这一问的（上面七类），护城河与模型强弱无关，模型越便宜反而越衬出它们的价值；经不起的——护城河只是"懂法规、会填表、会综述"——模型原生就会做，这层迟早被自助绕过，本质是 B 假扮的 C[[74]](https://a16z.com/good-news-ai-will-eat-application-software/)。一句话：**可持续的 C 是"AI 只是干活的手、护城河在别处"的生意；凡是"AI 本身就是全部价值"的，终将塌回 B**。

以下场景绝大多数落在 B 与 C：它们之所以长期空缺，恰恰因为过去没有任何单一买家能摊薄"做对一次"的成本。每个场景最终会沉淀成一个自助功能（B）、还是长成一门独立生意（C），要看它头顶那层护城河立不立得住——而不是看它属于哪个领域。

下面 §3.2.1.2–§3.2.1.5 的 45 个场景按这七类护城河的逐一归类、争议项、以及尚未列入的候选需求，见附件 [《45 场景 × 护城河类型 交叉分类》](moat-classification.md)。

把这套 A/B/C + 七类护城河框架套到真实上市公司的一个案例：2026 年初美股软件股重挫名单里 40 家 SaaS"若把卖席位的旧业务改造成卖 AI 劳动力，会落进哪一类、靠哪种护城河守住"，且框架预测的"塌 B"名单与跌幅最深的名单高度重合，见 [《软件股的"AI 劳动力"模型分析》](软件股AI劳动力改造类型%20(2026-05-30).md)。
...
5. **写迁移脚本的平台工程师**：Java 8→17、React 16→18、Python 2→3 这类迁移过去是"集中花一年时间的大项目"——参见 §2.7.1 与 §3.2.1.8 的 Google 内部迁移、Spotify×Anthropic、Mechanical Orchard 案例。
...
⚠ 解读：候选生成的数量级是真实的（GNoME 38 万稳定结构、736 已合成），但"新材料"标签经过同行复核后被显著打折。"AI 让材料发现提速 50–100 倍"是当事团队设定的目标，**不是已被复核的事实**。这本身就是 §2.7.1、§2.7.3 讲的"求解贵、纠错也贵"的现实案例：没有传统的人类同行评审、独立复现、晶体学数据库交叉核对——这套"传统计算机骨骼"——AI 的"暴力破解"很容易变成"暴力幻觉"。
