...
§3.2.1 回答了"需求在哪里"；接下来的问题是——找到需求之后，怎样运用 AI 劳动力满足需求。§3.2.1.6 末尾已经给出总纲：一旦把 Agent 建模为"AI 劳动力"而非传统软件系统或组件，组织它的就不是传统算法和软件架构，而是组织人的管理学与工程学方法论——提供支持、设定规范、沟通透明、严格质检、重点监督。本节以最典型、训练数据最丰富、当前 AI 能力最强的知识工作——**软件开发**——为标本，对比传统流程，提炼这套运用方法。

传统软件开发的标准流程，无论瀑布还是敏捷，去掉迭代节奏的差异，其工序拓扑是稳定的——一条从需求到运维的主链路，外加若干条返工与反馈回流：
...
graph TD
    ...
    Q1 -- 是 --> U[直接使用]
    Q2 -- 是 --> U
    Q2 -- 否：不满足的部分 --> D[系统设计与架构]
    D --> C[编码实现]
    C --> CR[代码评审]
    CR --> IG1
    subgraph IGSP [构建与集成]
        direction TB
        IG1[经典系统设计与架构] --> IG2[虚拟前置部署工程师（FDE）]
    end
    IG2 --> T[测试与质量保障]
    T --> DP[部署发布]
    DP --> MO[运行监控与维护]
