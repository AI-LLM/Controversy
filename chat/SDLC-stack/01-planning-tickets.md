...
- 一张 Jira ticket 的"生命周期"远长于上面的"cycle time"——后者只覆盖第一次 commit 到上线。一张典型业务 ticket 的完整生命：**PM 起草 → triage → backlog → refinement → sprint planning → 开发 → review → QA → release**，跨度通常 2–8 周（⚠ 行业经验值，作者综合估算；依据：Atlassian 推荐 sprint 2 周，backlog refinement 通常每 sprint 一次，多数业务 ticket 会跨 1–3 个 sprint 才完成开发 + QA + release [[31]](https://www.atlassian.com/agile/tutorials/sprints), [[32]](https://www.atlassian.com/agile/project-management/backlog-refinement-meeting)）。
...
1. **单张 ticket 的实际开发耗时从"几天"压到"30 分钟–几小时"**。一个 well-scoped ticket 由 Devin 类 agent 在 30 分钟内消费完是常态（⚠ 解读；依据：Cognition 公开数据点显示 Devin 处理安全漏洞修复 ~1.5 分钟/张、ETL 单文件迁移 3–4 小时，1 ACU ≈ 15 分钟有效执行 [[33]](https://cognition.ai/blog/how-cognition-uses-devin-to-build-devin)，"30 分钟内"为这两端之间的常见 well-scoped 任务区间估算）。
...
5. **工单数量净增**。每个原本的"大 ticket"被切成 3–10 个 agent-sized 任务（⚠ 行业经验值，作者综合估算；依据：Linear Agent 自报 3 个月 agent-driven 工作量 5× [[11]](https://buttondown.com/verified/archive/the-death-of-the-ticket-why-linear-is-pivoting/) + Devin/Cursor 公开的 ACU/PR 单位推算），加上 agent 自己生成的 follow-up（"我修了 A 但发现 B 也得修，已开新 ticket"），**工单数量不是降，是涨**。这跟 namespace.so 在 CI 层看到的"PR 数量爆炸"是同源现象。
