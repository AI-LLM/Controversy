...
模型退市后自动让位给次高价。这样看到的是各品牌天花板价随时间的变化。
...
- 时间平移：三条线各自起点对齐到 token 2020-06，
  - 商业计算机 1975-01 Altair 8800 → 2020-06（+545 月，45 年），最后点 2026-06 落到 token 2071-06；
...
- 等比缩放：三条线起点价 ≡ 2020-06 GPT-3 Davinci $60/1M tokens，
  ...
  - token 不缩放。
- Y 轴 log：商业计算机 45 年 $/MIPS 从 $1242 跌到 ~$1.47e-5（≈8400 万倍），
...
- 红虚线：商业计算机 $/MIPS，方形 marker（从 1975 Altair 8800 起；早于此的大型机
  数据见下表）。
- 灰虚线：互联网 $/Mbps，圆形 marker。
- 彩实线：各家 token API 最贵模型 blended。

观感：log 空间里 45 年商业计算机 $/MIPS 曲线接近完美指数下行——可清晰看到 1995 Pentium
超标量切一刀、2000 千兆赫兹再切一刀、2015+ 多核 + AVX 再降几阶、2026 NPU 跳一阶。
互联网斜率明显比计算机缓；而 token 最初 6 年几乎横盘——OpenAI $60 → $90 → $112.5 → $50
震荡一圈净跌 ~17%，Anthropic $45 横盘到 $15。**只跟1975-1984早期PC呈现相似模式**，DeepSeek为代表的离群样本也有C64的案例对应。
...
token 单价跌了，但单任务消耗的 token 量同期也涨了——这是判断"用户实际净支出"是否真降的
...
- 模型退市时间来自 CSV notes + Reddit 退市帖 + 官方公告，部分近似（±1 月）。
...
- 互联网 $/Mbps 由"名义月费 ÷ 该时段典型下行速率"算出，未做通胀调整。
  - 拨号时代速率用主流 modem 标准估算（1993-1995 取 14.4k = 0.0144 Mbps、1995-1996 取
  ...
  - 宽带初期速率（2000–2005）用入门 tier 典型值：2000-06 取 4.5 Mbps（早期宽带 3–6 Mbps
    ...
    取值会让 $/Mbps 上下浮动 ~2×。
  - 2015 后速率采用 USTelecom Broadband Pricing Index 的 avg 下行——这是带宽与套餐价
    最权威的年度配对数据，置信度最高。
