...
5. **市场本质在按"剥离 AI 后还剩什么护城河"重排座次**：把这份名单逐家套进 AI 劳动力的 A/B/C + 七类护城河框架后，"只剩席位 + UI"的塌向 B、攥着数据 / 网络 / 信任 / 合规的守在 C、卖算力数据的是受益的"骨骼"——而**框架预测的"塌 B"名单与跌幅最深的名单高度重合**。逐家归类见 [《软件股若改造成"卖 AI 劳动力"会落进哪一类》](软件股AI劳动力改造类型%20(2026-05-30).md)。
...
| **D6.6 产品供给侧 Agent 接口** | 不存在 | 三路径并行：CLI 化（Stripe Agent Toolkit、Vercel AI SDK、OpenCLI、CLI-Anything）、产品 MCP 化（GitHub / Sentry / Postgres / Linear / Atlassian / Cloudflare / Notion / Slack / Figma / Anthropic Agent Skills）、浏览器 Agent 化（Computer Use、Operator、Browserbase、Manus、Skyvern、Vercel agent-browser）；**反 Bot 经济学反转**（Cloudflare HTTP 402 pay-per-crawl）+ **社区强 wrap**（vendor 不能"装睡"）双轨并行 | 协议开源，价值在网关 / per-call / HTTP 402 pay-per-crawl | 各 SaaS 的 MCP 端、Cloudflare、Stripe、Anthropic、Browserbase、Manus；**+ 第三方 wrapper 社区** | iPaaS / 自定义 webhook 集成代码；纯 GUI / 仅 REST 的传统 SaaS |
...
每层的代表厂商、流量模式突变、新需求、技术架构、配置示例与 benchmark 详见子目录 [`chat/SDLC-stack/`](SDLC-stack/)（索引见 [`SDLC-stack/README.md`](SDLC-stack/README.md)），合计 ~4 万字、240+ IEEE 引用，全部为 2025–2026 一手数据。
