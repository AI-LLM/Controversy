...
- **机器可执行的策略（"每个 workflow 是策略"的物化）**：
  - [.github/policies/merge-policy.yaml](https://github.com/AI-LLM/console/blob/main/.github/policies/merge-policy.yaml)：把 PR 接受规则写成 machine-enforceable rules——`dco-required`、`adopters-human-approval`（`auto_merge: false`，[:11](https://github.com/AI-LLM/console/blob/main/.github/policies/merge-policy.yaml#L11)）、`no-direct-main`、`ci-gate`（required_checks build+lint，[:24](https://github.com/AI-LLM/console/blob/main/.github/policies/merge-policy.yaml#L24)）。
